...
export default function DiagnosticForm() {
  ...
  const handleCheckboxChange = (e: any, field: "mainWorry" | "operationStress") => {
  ...
  const handleNextStep = () => {
    ...
    if (step === 1) {
      if (!formData.industry) return setStepError("「1. 業種」を選択してください。");
      if (formData.industry === "その他" && !formData.industryOther) return setStepError("「その他」の具体的な業種をご記入ください。");
      if (formData.mainWorry.length === 0) return setStepError("「2. 最大の悩み」を1つ以上選択してください。");
    ...
    if (step === 2) {
      if (formData.operationStress.length === 0) return setStepError("「5. 現場のストレス」を1つ以上選択してください。");
      if (formData.operationStress.includes("その他") && !formData.operationStressOther) return setStepError("「その他」の具体的なストレスをご記入ください。");
      if (!formData.waitingTime) return setStepError("「6. 待ち時間」を選択してください。");
      if (formData.waitingTime === "その他" && !formData.waitingTimeOther) return setStepError("「その他」の待ち時間をご記入ください。");
      if (!formData.googleReviewStatus) return setStepError("「7. Google口コミの現状」を選択してください。");
  ...
  const handleSubmit = async (e: any) => {
    ...
    const payload = {
      ...
      mainWorry: formData.mainWorry.includes("その他")
        ? [...formData.mainWorry.filter(i => i !== "その他"), `その他（${formData.mainWorryOther}）`].join(" / ")
        : formData.mainWorry.join(" / "),
      ...
      operationStress: formData.operationStress.includes("その他")
        ? [...formData.operationStress.filter(i => i !== "その他"), `その他（${formData.operationStressOther}）`].join(" / ")
        : formData.operationStress.join(" / "),
      ...
      <div className="min-h-screen font-serif text-slate-200 bg-[#080d1a] flex flex-col items-center justify-center p-6">
        <div className="max-w-xl w-full bg-white/5 border border-white/10 p-10 rounded-sm text-center">
          ...
          <p className="text-sm text-white/60 leading-loose mb-8">
            ...
            後日、担当の沖田より診断レポートをお送りいたしますので、今しばらくお待ちください。
          </p>
        ...
        <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-sm relative grid">
          ...
          <div className={`col-start-1 row-start-1 transition-all duration-700 p-6 md:p-12 ${step === 1 ? "opacity-100 translate-x-0 z-10 pointer-events-auto" : "opacity-0 -translate-x-10 z-0 pointer-events-none"}`}>
            ...
            <div className="space-y-10">
              ...
              <div>
                <label className="block text-sm text-white/80 tracking-widest mb-4">1. お店の業種を教えてください（1つ選択）</label>
              ...
              <div>
                <label className="block text-sm text-white/80 tracking-widest mb-4">2. 現在、集客や売上における悩みは何ですか（複数選択可）</label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {["新規の来店数が足りない", "客単価が低く、利益が出にくい", "リピート客がなかなか定着しない", "Web集客（SNSやサイト）に手が回らない", "その他"].map((opt) => (
...
次へ進む →
          ...
          <div className={`col-start-1 row-start-1 transition-all duration-700 p-6 md:p-12 ${step === 2 ? "opacity-100 translate-x-0 z-10 pointer-events-auto" : "opacity-0 translate-x-10 z-0 pointer-events-none"}`}>
            <h2 className="text-lg text-white tracking-widest mb-8 border-b border-white/10 pb-4">STEP 2：現場の状況について</h2>
            <div className="space-y-10">
              
              <div>
                <label className="block text-sm text-white/80 tracking-widest mb-4">5. 現場オペレーションでストレスに感じることは何ですか（複数選択可）</label>
                <div className="grid grid-cols-1 gap-3">
                  {["電話対応や予約管理に時間を取られている", "スタッフの基本行動（挨拶・清掃など）の指導が難しい", "クレーム対応や、合わないお客様の対応に疲弊している", "特にストレスはなく、スムーズに回っている", "その他"].map((opt) => (
              ...
              <div>
                <label className="block text-sm text-white/80 tracking-widest mb-4">6. お客様の「待ち時間」が最も発生するのはいつですか（1つ選択）</label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {["ご案内前（入店時の順番待ちなど）", "商品やサービスの提供待ち", "お会計や退店準備のタイミング", "ほぼ待ち時間は発生していない", "その他"].map((opt) => (
              ...
              <div>
                <label className="block text-sm text-white/80 tracking-widest mb-4">7. Google口コミの現状を教えてください（1つ選択）</label>
                <div className="grid grid-cols-1 gap-3">
                  {["評価4.0以上で、定期的に件数も増えている", "評価3.5〜3.9付近で、あまり件数が伸びていない", "ネガティブな投稿があり、全体の評価が下がっている", "まだ登録していない、または全く手をつけていない", "その他"].map((opt) => (
          ...
          <div className={`col-start-1 row-start-1 transition-all duration-700 p-6 md:p-12 ${step === 3 ? "opacity-100 translate-x-0 z-10 pointer-events-auto" : "opacity-0 translate-x-10 z-0 pointer-events-none"}`}>
            <h2 className="text-lg text-white tracking-widest mb-8 border-b border-white/10 pb-4">STEP 3：理想の未来について</h2>
            <div className="space-y-10">
              
              <div>
                <label className="block text-sm text-white/80 tracking-widest mb-4">8. 今後、お店をどのように成長させたいですか（1つ選択）</label>
                <div className="grid grid-cols-1 gap-3">
